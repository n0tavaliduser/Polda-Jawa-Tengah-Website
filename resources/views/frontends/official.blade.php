                    ...
                    <div class="flex w-full h-20 pl-14 items-center bg-gray-600">
                        <form method="GET" class="-mt-2">   
                            ...
                            <div class="relative w-96">
                                ...
                                <input type="search" id="namesearch" name="namesearch" class="block w-full p-4 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-transparent focus:border-0" placeholder="Search">
                    ...
                    @foreach ($official as $item)
                        <div class="card w-full glass">
                            <div class="card-body p-4">
                                <img class="h-56" src="{{ asset('images/' . $item->image) }}" alt="">
                                <h2 class="card-title text-sm text-black">{{ $item->position }}</h2>
                                {{-- <p>How to park your car at your garage?</p> --}}
                                <div class="flex w-full h-full justify-center items-end">
                                    <label for="my-modal-{{ $item->id }}" class="btn">Detail</label>
                                </div>
                            </div>
                        </div>
                        ...
                        <label for="my-modal-{{ $item->id }}" class="modal cursor-pointer">
                            <label class="modal-box relative" for="">
                                <h3 class="text-lg font-bold"><img class="rounded-md shadow-md w-full" src="{{ asset('images/' . $item->image) }}" alt=""></h3>
                                <p class="pt-5 pb-2 text-2xl font-bold">Profil Detail</p>
                                <p class="py-1">Nama : {{ $item->name }}</p>
                                <p class="py-1">Posisi : {{ $item->position }}</p>
                                <p class="py-1">Kesatuan : {{ $item->unit }}</p>
                                <div class="flex justify-end">
                                    <label for="my-modal-{{ $item->id }}" class="btn">Close</label>
                                </div>
                            </label>
                        </label>
                    @endforeach
