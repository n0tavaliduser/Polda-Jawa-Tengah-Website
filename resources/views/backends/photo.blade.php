            ...
            <div class="overflow-x-auto relative shadow-md sm:rounded-lg">
                ...
                <div class="overflow-x-auto">
                    <table class="table w-full">
                        ...
                        <tbody>
                            @foreach ($photo as $key => $gallery)
                            <tr>
                                <td><img class="max-h-20" src="{{ asset('images/' . $gallery->gambar) }}" alt=""></td>
                                <th>{{ $gallery->title }}</th>
                                ...
                                <td class="flex gap-2 justify-center">
                                    ...
                                    <div id="confirm-delete-{{ $gallery->id }}" class="absolute h-screen w-full left-0 z-50 hidden top-0">
                                        <div class="alert shadow-lg w-fit">
                                            ...
                                            <div class="flex-none">
                                                ...
                                                <form method="post" action="{{ route('galleryphoto.destroy', $gallery->id) }}">
                                                    @csrf
                                                    @method('delete')
                                    ...
                                    <div class="modal" id="modal-{{ $gallery->id }}">
                                        <div class="modal-box">
                                            ...
                                            <img class="pb-5" src="{{ asset('images/' . $gallery->gambar) }}" alt="">
                                            <form method="post" action="{{ route('galleryphoto.update', $gallery->id) }}" enctype="multipart/form-data">
                                                @csrf
                                                @method('put')
                                                <div class="relative z-0 mb-6 w-full group">
                                                    <input type="file" name="gambar" id="gambar" value="{{ $gallery->gambar }}" class="block py-2.5 px-0 w-full text-sm text-black bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-black dark:border-gray-600 dark:focus:border-white-500 focus:outline-none focus:ring-0 focus:border-white-600 peer" value="{{ $home->herotext ?? old('herotext') }}" placeholder=" "/>
                                                    <label for="gambar" class="peer-focus:font-medium absolute text-sm text-black dark:text-black duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-black-600 peer-focus:dark:text-black-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Image</label>
                                                </div>
                                                <div class="relative z-0 mb-6 w-full group">
